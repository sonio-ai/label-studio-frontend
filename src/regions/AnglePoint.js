...
const AnglePointAbsoluteCoordsDEV3793 = types.model()
  .volatile(() => ({
    relativeX: 0,
    relativeY: 0,
    initX: 0,
...
const AnglePointRelativeCoords = types
  .model('AnglePoint', {
  ...
  .actions(self => ({
    /**
     * External function for Angle Parent
     * @param {number} x
     * @param {number} y
     */
    ...
    handleMouseOverStartPoint(ev) {
      ...
      /**
       * Check if angle > 2 points and closed point
       */
...
const AnglePoint = isFF(FF_DEV_3793)
  ? AnglePointRelativeCoords
  : types.compose('AnglePoint', AnglePointRelativeCoords, AnglePointAbsoluteCoordsDEV3793);

const AnglePointView = observer(({ item, name }) => {
    ...
    return (
      <Circle
        ...
        onClick={ev => {
          ...
          // don't unselect angle on point click
...
export { AnglePoint, AnglePointView };
