...
const Model = types
  ...
  .views(self => ({
    ...
    get sonioAngle() {
      ...
      const computeAngle = (line1, line2) => {
        const dAx = line1.end.x - line1.start.x;
        const dAy = line1.end.y - line1.start.y;
        const dBx = line2.end.x - line2.start.x;
        const dBy = line2.end.y - line2.start.y;
      ...
      const points = self.points?.map(p => ({x: p.x/100 * taskData.image_width, y: p.y/100 * taskData.image_height})) || [];
      let angle = false;
      if(points.length === 4) {
        angle = computeAngle({
